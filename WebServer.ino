...
void WebServerInit()
{
  ...
  WebServer.on("/configall", handle_configall);
  ...
  WebServer.on("/firmware", HTTP_GET, handle_upload);
  WebServer.on("/firmware", HTTP_POST, handle_upload_post, handleFileUpload);
  ...
  WebServer.on("/wifisetup", handle_wifisetup);
  ...
  WebServer.begin();
  IPAddress ip = WiFi.localIP(); char str[20];
  sprintf_P(str, PSTR("%u.%u.%u.%u"), ip[0], ip[1], ip[2], ip[3]);
  Serial.println("WebS : Webserver Started!@"+String(str));
...
void addHeader(boolean showMenu, String& str)
{
  ...
  if (!cssfile)
  {
    ...
    str += F("h1 {font-size:16pt; color:#e60000;}");
    str += F("h6 {font-size:10pt; color:#e60000; text-align:center;}");
    ...
    str += F("th {padding:10px; background-color:#000066; color:#ffffff;}");
  ...
  str += F("<h1>�?อต�?อ�?รั�?สู�? mini-NodeMCU: ");
  str += Settings.Name;
  ...
  if (showMenu)
  {
    str += F("<BR><a class=\"button-menu\" href=\".\">�?�?อมูลระ�?�?</a>");
    str += F("<a class=\"button-menu\" href=\"config\">ตั�?�?�?�?า WiFi</a>");
...
//    str += F("<a class=\"button-menu\" href=\"tools\">Tools</a><BR><BR>");
    str += F("<a class=\"button-menu\" href=\"mqtt\">MQTT/ThinkSpeak</a>");
    str += F("<a class=\"button-menu\" href=\"api\">API</a>");
//    str += F("<a class=\"button-menu\" href=\"firmware\">Firmware</a>");
    str += F("<a class=\"button-menu\" href=\"pin\">�?า Pin</a>");
    str += F("<a class=\"button-menu\" href=\"devices\">อุ�?�?รณ�?</a>");
    str += F("<a class=\"button-menu\" href=\"log\">Data Flow</a>");
  }
}
...
{
  str += F("<h6>Powered by mini-NodeMCU IoT Lite</h6></body>");
}
...
void handle_root() {
  ...
  if (wifiSetup)
  {
    ...
    WebServer.send(200, "text/html", "<meta HTTP-EQUIV='REFRESH' content='0; url=http://192.168.4.1/wifisetup'>");
    return;
  }
  ...
  if ((strcasecmp_P(sCommand.c_str(), PSTR("wifidisconnect")) != 0) && (strcasecmp_P(sCommand.c_str(), PSTR("reboot")) != 0))
  {
    ...
    reply += F("<TR><TH>สาร�?ั�?�?ห�?ด:<TH>IP<TH>Age<TR><TD><TD>");
...
void handle_config() {
  ...
  reply += F("<TH>ตั�?�?�?�?า WiFi<TH><TR style=\"background-color:#ffd480; color:black;\"><TD>Name:<TD><input type='text' name='name' value='");
  Settings.Name[25] = 0;
  reply += Settings.Name;
  reply += F("'> : %nodename%<TR style=\"background-color:#ffd480; color:black;\"><TD>Admin Password:<TD><input type='text' name='password' value='");
  SecuritySettings.Password[25] = 0;
  reply += SecuritySettings.Password;
  reply += F("'><TR style=\"background-color:#6699ff; color:black;\"><TD>SSID:<TD><input type='text' name='ssid' value='");
  reply += SecuritySettings.WifiSSID;
  reply += F("'><TR style=\"background-color:#6699ff; color:black;\"><TD>WPA Key:<TD><input type='text' maxlength='63' name='key' value='");
  reply += SecuritySettings.WifiKey;

  reply += F("'><TR style=\"background-color:#aa80ff; color:black;\"><TD>WPA AP Mode Key:<TD><input type='text' maxlength='63' name='apkey' value='");
  reply += SecuritySettings.WifiAPKey;

  reply += F("'><TR><TD>�?ห�?ดที�?:<TD><input type='text' name='unit' value='");
  reply += Settings.Unit;
  ...
  reply += F(" [ต�?อ�?า D11(GPIO16)เ�?�?า�?ั�? Reset(REST)]");



  reply += F("<TR><TH>ตั�?�?�?�?า�?�?�? Fixed IP<TH>");
  reply += F("<TR><TD>Fixed IP:<TD><input type='text' name='espip' value='");
  sprintf_P(str, PSTR("%u.%u.%u.%u"), Settings.IP[0], Settings.IP[1], Settings.IP[2], Settings.IP[3]);
  reply += str;

  reply += F("'><TR><TD>Gateway:<TD><input type='text' name='espgateway' value='");
  ...
  reply += F("'><TR><TD><TD><input class=\"button-link\" type='submit' value='ต�?ล�?'>");
  reply += F("</table></form>");
  addFooter(reply);
  WebServer.send(200, "text/html", reply);
}
...
//********************************************************************************
// Web Interface configall page
//********************************************************************************
void handle_configall() {
  if (!isLoggedIn()) return;
  ...
  if (Settings.Protocol)
  {
    ...
    if (Settings.UseDNS)
    {
      reply += F("<TR><TD>Controller Hostname:<TD><input type='text' name='controllerhostname' size='30' value='");
...
void handle_mqtt() {
  ...
  if (edit.length() != 0) {
    char tmpString2[81];
    mqttpublish.toCharArray(tmpString2, 81);
    strcpy(Settings.MQTTpublish, tmpString2);
  }
  ...
  if (ssid[0] != 0)
  {
    ...
    if (Settings.Protocol != protocol.toInt())
    {
      Serial.println("123Protocal :"+protocol+":"+D4);
      Settings.Protocol = protocol.toInt();
      byte ProtocolIndex = getProtocolIndex(Settings.Protocol);
      Settings.ControllerPort = Protocol[ProtocolIndex].defaultPort;
      if (Protocol[ProtocolIndex].usesMQTT)
  ...
  reply += F("<form name='frmselect' method='post'><table>");
  reply += F("<TH>ตั�?�?�?�?า IoT Server<TH><TR style=\"display:none;\"><TD>Name:<TD><input type='text' name='name' value='");
  Settings.Name[25] = 0;
  reply += Settings.Name;
  reply += F("'><TR style=\"display:none;\"><TD>Admin Password:<TD><input type='text' name='password' value='");
  SecuritySettings.Password[25] = 0;
  reply += SecuritySettings.Password;
  reply += F("'><TR style=\"display:none;\"><TD>SSID:<TD><input type='text' name='ssid' value='");
  reply += SecuritySettings.WifiSSID;
  reply += F("'><TR style=\"display:none;\"><TD>WPA Key:<TD><input type='text' maxlength='63' name='key' value='");
  reply += SecuritySettings.WifiKey;

  reply += F("'><TR style=\"display:none;\"><TD>WPA AP Mode Key:<TD><input type='text' maxlength='63' name='apkey' value='");
  reply += SecuritySettings.WifiAPKey;

  reply += F("'><TR style=\"display:none;\"><TD>Unit nr:<TD><input type='text' name='unit' value='");
  reply += Settings.Unit;
  ...
  if (Settings.Protocol)
  {
    ...
    optionValues[0] = 0;
    optionValues[1] = 1;
    reply += F("<TR><TD>Host by:<TD><select name='usedns' LANGUAGE=javascript onchange=\"return dept_onchange(frmselect)\" >");
    for (byte x = 0; x < 2; x++)
    {
      reply += F("<option value='");
      reply += optionValues[x];
    ...
    if (Settings.UseDNS)
    {
      reply += F("<TR><TD>IoT Hostname:<TD><input type='text' name='controllerhostname' size='30' value='");
      reply += Settings.ControllerHostName;
    }
    else
    {
      reply += F("<TR><TD>IoT IP:<TD><input type='text' name='controllerip' value='");
      sprintf_P(str, PSTR("%u.%u.%u.%u"), Settings.Controller_IP[0], Settings.Controller_IP[1], Settings.Controller_IP[2], Settings.Controller_IP[3]);
      reply += str;
    }

    reply += F("'><TR><TD>IoT Port:<TD><input type='text' name='controllerport' value='");
    reply += Settings.ControllerPort;

    byte ProtocolIndex = getProtocolIndex(Settings.Protocol);
    if (Protocol[ProtocolIndex].usesAccount)
    {
      reply += F("'><TR><TD>IoT User:<TD><input type='text' name='controlleruser' value='");
      reply += SecuritySettings.ControllerUser;
    }

    if (Protocol[ProtocolIndex].usesPassword)
    {
      reply += F("'><TR><TD>IoT Password:<TD><input type='text' name='controllerpassword' value='");
      reply += SecuritySettings.ControllerPassword;
    }
    //reply += F("'>");
  }
...
void handle_hardware() {
  ...
  reply += F("<TR><TD><TD><input class=\"button-link\" type='submit' value='ต�?ล�?'><TR><TD>");
...
void handle_pin() {
  ...
  reply += F("<form  method='post'><table><TH>ตั�?�?�?�?า pin เริ�?มต�?�?<TH><TR><TD>");
  reply += F("<TR><TD>Led �?อ�?สถา�?ะ WiFi:<TD>");
  add_mnNM_PinSelect(false, reply, "pled", Settings.Pin_status_led);
  reply += F("<TR><TD>I2C �?า SDA:<TD>");
  add_mnNM_PinSelect(true, reply, "psda", Settings.Pin_i2c_sda);
  reply += F("<TR><TD>I2C �?า SCL:<TD>");
  add_mnNM_PinSelect(true, reply, "pscl", Settings.Pin_i2c_scl);

  reply += F("<TR><TD>สถา�?ะ�?า Pin เริ�?มต�?�?:<TD>");
  reply += F("<TR><TD>�?า D1(RED):<TD>");
  addPinStateSelect(reply, "d1", Settings.PinBootStates[D1]);
  reply += F("<TR><TD>�?า D2:<TD>");
  addPinStateSelect(reply, "d2", Settings.PinBootStates[D2]);
  reply += F("<TR><TD>�?า D3:<TD>");
  addPinStateSelect(reply, "d3", Settings.PinBootStates[D3]);
  reply += F("<TR><TD>�?า D4(BUTTON):<TD>");
  addPinStateSelect(reply, "d4", Settings.PinBootStates[D4]);
  reply += F("<TR><TD>�?า D5:<TD>");
  addPinStateSelect(reply, "d5", Settings.PinBootStates[D5]);
  reply += F("<TR><TD>�?า D6:<TD>");
  addPinStateSelect(reply, "d6", Settings.PinBootStates[D6]);
  reply += F("<TR><TD>�?า D7:<TD>");
  addPinStateSelect(reply, "d7", Settings.PinBootStates[D7]);
  reply += F("<TR><TD>�?า D8(BLUE):<TD>");
  addPinStateSelect(reply, "d8", Settings.PinBootStates[D8]);
  reply += F("<TR><TD>�?า D9(GREEN):<TD>");
  addPinStateSelect(reply, "d9", Settings.PinBootStates[D9]);
  reply += F("<TR><TD>�?า D10:<TD>");
  addPinStateSelect(reply, "d10", Settings.PinBootStates[D10]);
  reply += F("<TR><TD>�?า D11:<TD>");
...
void handle_devices() {
  ...
  else
  {
    ...
    reply += F("<BR><BR><form name='frmselect' method='post'><table><TH>ตั�?�?�?�?าอุ�?�?รณ�?<TH>Value");

    reply += F("<TR><TD>อุ�?�?รณ�?:<TD>");
    ...
    if (Settings.TaskDeviceNumber[index - 1] != 0 )
    {
      ...
      reply += F("<TR><TD>�?ื�?ออุ�?�?รณ�?:<TD><input type='text' maxlength='40' name='taskdevicename' value='");
      reply += ExtraTaskSettings.TaskDeviceName;
      reply += F("'> : %devicename%");
      ...
      if (!Device[DeviceIndex].Custom && Settings.TaskDeviceDataFeed[index - 1] == 0)
      {
        ...
        {
          reply += F("<TR><TD>�?า Pin ที�? 1:<TD>");
          add_mnNM_PinSelect(false, reply, "taskdevicepin1", Settings.TaskDevicePin1[index - 1]);
        }
        if (Device[DeviceIndex].Type == DEVICE_TYPE_DUAL)
        {
          reply += F("<TR><TD>�?า Pin ที�? 2:<TD>");
          add_mnNM_PinSelect(false, reply, "taskdevicepin2", Settings.TaskDevicePin2[index - 1]);
        }
      ...
      if (!Device[DeviceIndex].Custom)
      {
        reply += F("<TR><TH>ตั�?�?�?�?าเ�?ิ�?มเติม<TH>Value");
        ...
        if (Device[DeviceIndex].FormulaOption)
        {
          for (byte varNr = 0; varNr < Device[DeviceIndex].ValueCount; varNr++)
          {
            ...
            reply += F(" ทศ�?ิยม: <input type='text' name='taskdevicevaluedecimals");
            reply += varNr + 1;
            reply += F("' value='");
    ...
    reply += page;
    reply += F("\">�?ิด</a>");
    reply += F("<input class=\"button-link\" type='submit' value='ต�?ล�?'>");
    reply += F("<input type='hidden' name='edit' value='1'>");
...
//********************************************************************************
// Add a miniNodeMCU pin select dropdown list
//********************************************************************************
void add_mnNM_PinSelect(boolean forI2C, String& str, String name,  int choice)
{
  //String options[14];
  String options[12];
  options[0] = F(" ");
...
void handle_log() {
  ...
  reply += F("<table><TH>Data Flow<TR><TD>");
...
void handle_upload() {
  ...
  String reply = "";
  addHeader(true, reply);
  reply += F("<form enctype=\"multipart/form-data\" method=\"post\"><p>อั�?�?หลด Firmware:<br><br><input type=\"file\" name=\"datafile\" size=\"40\"></p><div><input class=\"button-link\" type='submit' value='Upload'></div></form>");
  addFooter(reply);
  WebServer.send(200, "text/html", reply);
  printWebString = "";
  printToWeb = false;
...
void handle_api() {
  ...
  String reply = ""; String url="";
  addHeader(true, reply);
  
  reply += F("<BR>");
  url = F("http://"); url += host; url += F("/control?cmd=GPIO,12,1");
  reply += F("<a href="); reply += url; reply += F(">"); reply += url; reply += F("</a><BR>");
  url = F("http://"); url += host; url += F("/control?cmd=GPIO,12,0");
  reply += F("<a href="); reply += url; reply += F(">"); reply += url; reply += F("</a><BR>");
...
void handle_setup() {
  ...
  if (status == 1)  // connecting stage...
  {
    ...
    else
    {
      ...
      if (refreshCount != 0)
        wait = 3;
      reply += F("�?รุณารอ <h1 id=\"countdown\">20..</h1>");
      ...
      reply += F(");");
      reply += F("</script>");
      reply += F("วิ�?าที �?ำลั�?เ�?ื�?อมต�?อ WiFi");
    }
    refreshCount++;
  }
...
//********************************************************************************
// Web Interface WiFi-Setup Wizard
//********************************************************************************
void handle_wifisetup() {
  ...
  if (WiFi.status() == WL_CONNECTED)
  {
    ...
    char host[20];
    sprintf_P(host, PSTR("%u.%u.%u.%u"), ip[0], ip[1], ip[2], ip[3]);
    reply += F("<BR>mini-NodeMCU เ�?ื�?อมต�?อสั�?�?าณ wifi สำเร�?�? มี�?�?า IP Address: ");
    reply += host;
    reply += F("<BR><BR>�?รุณาสลั�? �?อม�?ิวเตอร�?/�?�?�?ต�?ุ�?�?/มือถือ �?ลั�?สู�?เ�?รื�?อ�?�?าย WiFi หลั�?เดิม �?ล�?ว ");
    reply += F("<a class=\"button-menu\" href='http://");
    reply += host;
    reply += F("/config'>�?ดลิ�?�?�?�?ี�?เ�?ื�?อทำ�?ารตั�?�?�?�?า</a>");
    addFooter(reply);
    WebServer.send(200, "text/html", reply);
    wifiSetup = false;
    WifiAPMode(false);
...
void handle_rules() {
  ...
  reply += F("<TR><TD><input class=\"button-link\" type='submit' value='ต�?ล�?'>");
  reply += F("</table></form>");
  delete [] data;
  addFooter(reply);
  WebServer.send(200, "text/html", reply);
